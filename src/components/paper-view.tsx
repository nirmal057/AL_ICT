...
const SubQuestionView = ({ subPart, onNoteLinkClick, level = 0 }: { subPart: SubQuestion, onNoteLinkClick: (noteId: string) => void, level?: number }) => {
    return (
        <div key={subPart.id} className="sub-question" style={{ marginLeft: level > 0 ? 12 : 0 }}>
            ...
            {subPart.subParts && subPart.subParts.length > 0 && (
                <div className="nested-sub-questions mt-3 border-l pl-4">
                    {subPart.subParts.map(child => (
                        <SubQuestionView key={child.id} subPart={child} onNoteLinkClick={onNoteLinkClick} level={level + 1} />
                    ))}
                </div>
            )}
        </div>
    ...
    return (
        <div id={question.id} className="mb-8 break-inside-avoid structured-question">
            ...
            {question.subParts && question.subParts.map(subPart => (
                <SubQuestionView key={subPart.id} subPart={subPart} onNoteLinkClick={onNoteLinkClick} />
            ))}
